...
// 🔥 AUTOMATIC DATABASE PATCH: Adds the missing role column if it isn't there yet
$conn->query("ALTER TABLE project_members ADD COLUMN IF NOT EXISTS role ENUM('owner', 'member', 'viewer') DEFAULT 'member' AFTER user_id");
...
<head>
    ...
    <style>
        ...
        .member-card { background: white; padding: 10px; margin: 5px 0; border-radius: 4px; border-left: 4px solid #007bff; display:flex; justify-content: space-between; }
    </style>
</head>
<body>
<div class="container">
    ...
    <?php if(!empty($error)): ?><div class="alert alert-error" style="color:red;"><?php echo htmlspecialchars($error); ?></div><?php endif; ?>
    <?php if(!empty($success)): ?><div class="alert alert-success" style="color:green;"><?php echo htmlspecialchars($success); ?></div><?php endif; ?>

    <div class="team-box">
        <h3>Invite a Team Member</h3>
        <form method="POST">
            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(generateCSRFToken()); ?>">
            <select name="user_id" style="padding: 10px; width: 250px;">
                <?php foreach ($all_users as $u): ?>
                    <option value="<?php echo $u['id']; ?>"><?php echo htmlspecialchars($u['name']); ?></option>
                <?php endforeach; ?>
            </select>
            <button type="submit" name="add_member" class="btn btn-primary" style="padding:9px 15px;">Add to Project</button>
        </form>
    </div>
